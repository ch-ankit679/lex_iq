    ...
    <div className={`${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'} flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen`}>
      <div>
        <div className={`flex ${expand ? 'flex-row gap-10' : 'flex-col items-center gap-8'}`}>
          ...
          className='group relative flex items-center justify-center h-9 w-9 aspect-square-rounded-lg cursor-pointer'>
            ...
            <Image src={expand ? assets.sidebar_close_icon : assets.sidebar_icon} alt='' className='hidden md:block w-7 mt-8 hover:bg-gray-500/20 transition-all duration-300'/>
            <div className={`absolute w-max ${expand ? 'left-1/2 -translate-x-1/2 top-12': '-top-12 left-0'}
              ...
              <div className={`${expand ? 'left-1/2 -top-1.5 -translate-x-1/2' : 'left-4 -bottom-1.5'} w-3 h-3 absolute bg-black rotate-45`}></div>
            </div>
          </div>
        </div>
          <button onClick={createNewChat} className={`${expand ? 'bg-primary hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max':
            'group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg'} mt-8 flex items-center justify-center cursor-pointer`}>
      ...
      <div>
        <div className={`flex items-center cursor-pointer group relative ${
          expand ? 'gap-1 text-white/80 p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer' : 
          'h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg'}`}>
            <Image className={expand ? 'w-5' : 'w-6.5 mx-auto'} src={assets.github_mark} alt=''/>
